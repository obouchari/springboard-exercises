...
function vowelCount(str) {
  ...
  if (vowels !== null) {
    vowels.forEach(
      (vowel) =>
        (vowelsCount[vowel] = vowelsCount[vowel] ? vowelsCount[vowel] + 1 : 1)
    );
  }
...
function find(arr, searchValue) {
  const results = arr.filter((val) => val === searchValue);
  return results[0];
}
...
function findInObj(arr, key, searchValue) {
  const results = arr.filter((obj) => obj[key] === searchValue);
  return results[0];
}
...
function removeVowels(str) {
  //   return str.toLowerCase().replace(/[aeuio]/g, ""); // Using RegEx
